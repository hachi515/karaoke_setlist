...
function growthBadgeHtml(it, mode){
  ...
  const d = cur - prev;
  if(prev<=0 && cur>0) return `<span class="growth-badge new">NEW</span>`;
  if(prev<=0 && cur<=0) return '';
  const pct = Math.round((d/prev)*100);
  if(pct===0) return `<span class="growth-badge flat">±0%</span>`;
  if(pct>0) return `<span class="growth-badge up">▲${pct}%</span>`;
  return `<span class="growth-badge down">▼${Math.abs(pct)}%</span>`;
}
...
async function handleImageUpload(input, cat, work, thumbId){
  ...
  try{
    ...
    const existingId = (IMAGE_MAP[cat] && IMAGE_MAP[cat][work]) || '';
    const res = await fetch(GAS_URL, {
      method:'POST',
      headers:{'Content-Type':'text/plain;charset=utf-8'},
      body: JSON.stringify({action:'upload_image',quarter:cat,work:work,filename:file.name,content_base64:base64,mime:'image/jpeg',existing_file_id:existingId,overwrite:true})
    });
